...
public class AuditAspect {
    ...
    public AuditAspect(
            ...
            AuditMetadataFactory metadataFactory
    ) {
    ...
    public Object audit(
        ...
        String resourceId = evaluateResourceId(
                joinPoint,
                logAction.resourceId(),
                result
        );

        Map<String, Object> metadata = metadataFactory.create(
                action,
                joinPoint.getArgs(),
                result
        );
    ...
    private String evaluateResourceId(
            ProceedingJoinPoint joinPoint,
            String expression,
            Object result
    ) {
        ...
        String resourceIdExpression = Objects.requireNonNull(
                expression,
                "Audit resourceId expression must not be null"
        );

        Object value = parser
                .parseExpression(resourceIdExpression)
                .getValue(context);
