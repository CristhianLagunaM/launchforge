...
public class AdminUserService {
    ...
    public AdminUserService(UserRepository users, RoleRepository roles) { this.users = users; this.roles = roles; }

    @Transactional(readOnly = true)
    public List<AdminUserResponse> list() { return users.findAll().stream().map(AdminUserResponse::from).toList(); }

    @Transactional
    public AdminUserResponse update(UUID id, AdminUserUpdateRequest request) {
        var user = users.findById(id).orElseThrow(() -> new ApiNotFoundException("Usuario no encontrado", "No existe el usuario solicitado.", "users/not-found"));
        var role = roles.findByNameIgnoreCase(request.role()).orElseThrow(() -> new ApiNotFoundException("Rol no encontrado", "El rol indicado no existe.", "users/role-not-found"));
        user.setEnabled(request.enabled());
        user.getUserRoles().clear();
        var assignment = new UserRole();
        assignment.setUser(user); assignment.setRole(role);
        var key = new UserRoleId(); key.setUserId(user.getId()); key.setRoleId(role.getId()); assignment.setId(key);
